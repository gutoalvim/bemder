...
class ExteriorBEM:
    ...
    AG_init = ctrl.AlgControls(AP_init.c0, 1000,1000,10)
    ...
    def __init__(self,grid=grid_init,AG=AG_init,AP=AP_init,S=S_init,R=R_init,mu=None):
        self.grid = grid
        self.f_range = AG.freq
...
#            v_fun = bempp.api.GridFunction(self.space, fun=v_data)
            ...
            Y = (mu_op_r+1j*mu_op_i)# + monopole_fun
    ...
    def point_evaluate(self,boundData,R=R_init):
    ...
    def combined_grid_evaluate(self,boundData,fi=0,plane="xy",d=0,grid_size=[4,4],n_grid_pts=600):
...
def pack(self):
    # Simulation data
    grid = {'vertices': self.grid.vertices,
    ...
    for sol in range(len(self.boundData)):
    ...
    simulation_data = {'frequencies': self.f_range,
    ...
    for attr, value in vars(self).items():
        if str(attr) == 'Configuration' or str(attr) == 'simulation':
            pass
        else:
            bem_data[attr] = value
    ...
    return simulation_data, bem_data
...
def load(self, simulation_data, bem_data):
    ...
    for sol in range(len(simulation_data['u'])):
        u = GridFunction(simulation_data['space'], coefficients=simulation_data['u'][sol])
        un = GridFunction(simulation_data['space'], coefficients=simulation_data['un'][sol])
