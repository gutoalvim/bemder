...
def bem_load(filename,ext='.pickle'):
        ...
        if EoI ==0:
            ...
            return ExteriorBEM(gridpack,AC,AP,S,R,mu), bData
...
#            v_fun = bempp.api.GridFunction(self.space, fun=v_data)
    ...
    def bem_load(self, filename, ext = ".pickle"):
        ...
        bData = {}
        for sol in range(len(self.f_range)):
            u = bempp.api.GridFunction(self.space, coefficients=simulation_data['bd'][sol][0])
            un = bempp.api.GridFunction(self.space, coefficients=simulation_data['bd'][sol][1])
            # incident_traces = GridFunction(self.simulation._space, coefficients=simulation_data['incident_traces'][sol])
            bData[sol] = [u, un]
